...
Panel {
  ...
  readonly property string nowPlaying: {
    ...
    for (var i = 0; i < mprisPlayers.length; i++) {
      ...
      if (dbus.indexOf("playerctld") !== -1) continue
      if (!(p.trackTitle || p.trackArtist)) continue
      if (p.isPlaying) { player = p; break }
  ...
  function outputIcon(volume) {
    ...
    if (outputMuted) return isHeadphones(sink) ? "󰟎" : "" // Improved checks for headphones mute
  ...
  BarIconButton {
    ...
    onPressed: function(b) {
      if (b === Qt.RightButton) root.toggleAllMuted()
        ...
        Quickshell.execDetached(["omarchy-shell", "media", "playPause"]) // add play/pause via middle click
      else root.toggle()
    }
    ...
    onWheelMoved: function(delta) {
      ...
      var volume = root.setOutputVolume(root.outputVolume + wheel.steps * 0.01) // 1% per wheel notch
